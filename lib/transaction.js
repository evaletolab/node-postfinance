...
transaction.Transaction = function(opts) {
  ...
  if (!opts.transactionId) {
    ...
    this.data.currency = this.data.currency || config.option('currency');
  }
...
transaction.Transaction.prototype.process = function(card, callback) {
  ...
  if (!card || !card.token) {
    throw new DaimyoError('system', 'Card has no token', null);
  }

  // Augment transaction data with token
  self.data.paymentMethodToken = card.token;
