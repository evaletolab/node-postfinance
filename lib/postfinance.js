...
Card.prototype.getPayload = function(options) {
  ...
  var paymentData = {
    ...
    'CVC': this.csc||'',
    'SHASIGN': '',
    'COM': this.com&&JSON.stringify(this.com) || '',
    /*operations 
      RES: request auth, 
...
/**
 * ## postfinance.Card.create(callback)
 ...
 *
 * Instructs Postfinance to retain (save permanently) the payment method by
 ...
 * Example:
 *
 *     var cardData = {...}, tokenOpts={...};
 *     var card = new postfinance.Card(cardData);

 *     card.create(tokenOpts,function(err) {
 *       if (err) {
 *         console.log(err);
 *         return;
 *       }
...
Card.prototype.create = function(options, callback) {
  ...
  // already existing?
  if (this.payId) {
    return callback(null, this.payId);
  }

  // when creating alias no orderId
  if(!this.orderId||!options.orderId){
  ...
  authpost.makeRequest({
    ...
    if(paymentDataObj.ALIAS&&paymentDataObj.ALIAS!=res.body.ALIAS){
    ...
    self.alias = res.body.ALIAS||undefined;
