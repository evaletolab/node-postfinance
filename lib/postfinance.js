...
function Card(opts) {
  ...
  if (opts) {
    ...
    } else if(opts.paymentMethod||opts.inline){
 ...
 */
Card.prototype.getPayload = function(options, withForm) {
  ...
  //
  // do not serialize user/pwd for form
  if(!withForm){
    /*operations
      RES: request auth,
      SAL:riquest direct sale,
      RFD:refund */
    paymentData['OPERATION']=config.option('operation');
    paymentData['USERID']=config.option('apiUser');
    paymentData['PSWD']=config.option('apiPassword');    
  }
  // case of name
  ...
  var constraintFields=['HOMEURL','orderId','PAYID','payId','GLOBORDERID','email','OPERATION','amount','paramplus','complus','com','alias','aliasUsage',
                        'tp','title','bgcolor']
  if(options){
    ...
    Object.keys(options).forEach(function(key){
      ...
      if(key==='amount'){
        paymentData['AMOUNT']=Math.round(options.amount*100);
        return;
      }
  ...
  // sign the content
  paymentData.SHASIGN=check.shaSign(paymentData);
...
Card.prototype.publish = function(options, callback) {
  ...
  if (!options.alias &&!this.number&&!this.orderId&&!this.inline) {
  ...
  //
  // check aliasUsage
  if(options.alias &&!options.aliasUsage){
     return callback(new PostFinanceError('system', 'Vous devez spécifier l\'option Alias Usage.'));    
  }
  ...
  var paymentDataObj=this.getPayload(options,self.inline)
  ...
  if(self.inline){
    var result={
      url:'https://'+config.option('host')+config.option('path')['ecommerce'],
      body:paymentDataObj,
      query:paymentData
    }
