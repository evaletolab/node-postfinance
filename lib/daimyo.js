...
function Card(opts) {
  if (opts) {
    if (opts.token && tokenRe.test(opts.token)) {
      ...
      // Single-digit years are converted to Nth year of this decade
      if (this.year && this.year < 10) {
        this.year = (Math.floor(new Date().getFullYear() / 10) * 10) + this.year;
      }

      // Double-digit years are converted to Nth year of this century
      if (this.year && this.year < 100) {
        this.year = (Math.floor(new Date().getFullYear() / 100) * 100) + this.year;
      }
