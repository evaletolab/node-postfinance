 ...
 * Future versions of Daimyo may eliminate this property and trap error 
 * messages at a lower-level API. For now, this is the only place where
 * you can get the error messages from the server.
 *
...
Card.prototype.load = function(callback) {
  ...
  if (!this.token) {
    callback(new DaimyoError('system', 'Cannot load payment method without token', null));
  }
...
Card.prototype.update = function(callback) {
  ...
  updatedFields = daimyo.toSamurai(updatedFields);
