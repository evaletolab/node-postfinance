...
check.issuers = {};
check.issuers.VISA = 'Visa';
check.issuers.MAST = 'MasterCard';
check.issuers.DINA = 'Diners Club';
check.issuers.AMEX = 'American Express';
check.issuers.DISC = 'Discover';
check.issuers.JCB = 'JCB';
check.issuers.NONE = 'Unknown';
...
check.issuers.CHECK = {
  VISA: /^4(\d{12}|\d{15})$/,
  MAST: /^5[1-5]\d{14}$/,
  DINA: /^3(00|05|6\d|8\d)\d{11}$/,
  AMEX: /^3[47]\d{13}$/,
  DISC: /^(622[1-9]|6011|64[4-9]\d|65\d{2})\d{12}$/,
...
check.getIssuer = function(card) {
  ...
  var lastMatch = check.issuers.NONE;
  Object.keys(check.issuers.CHECK).forEach(function(issuer) {
    if (check.issuers.CHECK[issuer].test(card)) {
      lastMatch = issuer;
    }
  });
  return check.issuers[lastMatch] || check.issuers.NONE;
};
